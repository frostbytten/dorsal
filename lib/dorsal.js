...
//	winston.add(winston.transports.File, { filename: './dorsal.log' });
	...
	this.addCustomSongToRadio = function( sid, radio, song, callback ) {
		...
		this.rawCall(body, function( err, data ) {
			if( err == null ) {
				winston.log('info', 'Autoplay as of Add:', data);
				self.authenticate( sid, function( err, auth ) {
					winston.log('info', 'Authentication', data);
					var body = JSON.stringify({'method':'voteUpAutoplaySong','wsKey': this.key, 'sessionID': sid, 'autoplayState': data.autoplayState, 'song': cleanSong });
					winston.log("")
					self.rawCall(body, function( err, data ) {
						if( err == null ) {
								winston.log('info', 'autoplay as of voteup', data);
								var body = JSON.stringify({'method':'removeSongFromAutoplay','wsKey': this.key, 'sessionID': sid, 'autoplayState': data.autoplayState, 'song': cleanSong });
								self.rawCall(body, function( err, data ) {
									winston.log('info', 'As of the final call', data);
									if( err == null ) {
										callback(null, data);
									} else {
										callback(err, null);
    ...
    this.searchMultiArtists = function( sid, artists, options, callback ) {
        ...
        for( var i=0; i < c; i++) {
			var artist = artists[i].replace(/[^\u0000-\u0080]/g, '');
			body = JSON.stringify({'method':'getArtistSearchResults', 'wsKey': this.key, 'sessionID': sid, 'query': artist, limit: limit});
            this.rawCall(body, function( err, data ) {

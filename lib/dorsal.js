...
function GSAPI(key, secret, username, hmac) {
	...
	this.upVote = function( sid, radio, song, callback ) {
		var cleanSong = {SongID: song.SongID, AlbumID: song.AlbumID, ArtistID: song.ArtistID},
		    body = 	JSON.stringify({'method': 'removeVoteUpAutoplaySong', 'wsKey': this.key, 'sessionID': sid, 'autoplayState': radio, 'song': cleanSong} );
		this.rawCall(body, callback);
	}
	
	this.downVote = function( sid, radio, song, callback ) {
		var cleanSong = {SongID: song.SongID, AlbumID: song.AlbumID, ArtistID: song.ArtistID},
		    body = 	JSON.stringify({'method': 'removeVoteDownAutoplaySong', 'wsKey': this.key, 'sessionID': sid, 'autoplayState': radio, 'song': cleanSong} );
		this.rawCall(body, callback);
	}
